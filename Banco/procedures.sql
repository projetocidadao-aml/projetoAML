...
/*
PROCEDIMENTO UTILIZADO PARA PRÉ CADASTRAR, GERANDO AUTOMATICAMENTE O PROTOCOLO PARA HANDSHAKE.
*/
...
SELECT *, date_format(HISTORICO_DATA, '%d/%m/%Y') as DATA_HISTORICO, date_format(HISTORICO_DATA, '%H:%i') as HORA_HISTORICO, lower(MIDIA_TIPO_ARQUIVO) as MIDIA 
 FROM HISTORICO_DESAFIO HD 
 INNER JOIN GRUPO GR ON GR.GRUPO_ID = HD.GRUPO_ID 
 inner join POST_MIDIA PM on PM.HISTORICO_ID = HD.HISTORICO_ID
 WHERE GR.ID_ETEC = COD_ETEC ORDER BY HD.HISTORICO_ID DESC LIMIT PAGINACAO, 3;
...
DELIMITER $$
CREATE PROCEDURE USP_UPLOAD_PRE_CADASTRO(IN NOME_PRE_CADASTRO VARCHAR(100), IN EMAIL_PRE_CADASTRO VARCHAR(100),
IN CPF_PRE_CADASTRO VARCHAR(15), IN PERFIL_PRE_CADASTRO VARCHAR(15), IN CURSO_PRE_CADASTRO INT, IN SEMESTRE_PRE_CADASTRO INT)
BEGIN
	...
	START TRANSACTION;
		...
		SET IDPERFIL = (SELECT PERFIL_ID FROM PERFIL WHERE PERFIL LIKE PERFIL_PRE_CADASTRO);
        
		INSERT INTO PESSOA (PESSOA_NOME,PESSOA_EMAIL,PESSOA_CPF,ID_PERFIL) 
					VALUES (NOME_PRE_CADASTRO,EMAIL_PRE_CADASTRO,CPF_PRE_CADASTRO, IDPERFIL);
		
        SET IDPESSOA = last_insert_id();
        
        INSERT INTO PESSOA_CURSO(ID_PESSOA, ID_CURSO, PESSOA_CURSO_SEMESTRE) 
					VALUES (IDPESSOA,CURSO_PRE_CADASTRO,SEMESTRE_PRE_CADASTRO);
        
		IF exception = 1
			THEN
				SELECT 'Erro ao cadastrar pessoas' as Msg;
                ROLLBACK;
			ELSE
            COMMIT;
		END IF;
...
CALL USP_UPLOAD_PRE_CADASTRO('GUSTAVO SILVA','TESTE@TESTE.COM.BR','123.456.789-00','Aluno', 1,1);

CALL USP_ATIVIDADES(1,0);
